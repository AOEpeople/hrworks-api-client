...
data class OrganizationUnitList(val organizationUnits: List<OrganizationUnit>)
data class AbsenceTypeList(val absenceTypes: List<AbsenceType>)


data class OrganizationUnit(val organizationUnitNumber: String,
                            val organizationUnitName: String)

data class AbsenceData(val beginDate: Date,
                       val endDate: Date,
                       val absences: List<Absence>)

data class AccumulatedAbsenceData(val beginDate: Date,
                                  val endDate: Date,
                                  val absences: Map<String, Double>)

data class Availability(val beginDate: Date, val endDate: Date, val workingHours: Double)

data class LeaveAccountData(val holidayEntitlement: Int,
                            val requested: Int,
                            val approved: Int,
                            val unplanned: Int,
                            val planned: Int)

data class Person(val personnelNumber: String,
                  val personId: String,
                  val firstName: String,
                  val lastName: String)

data class AbsenceType(val name: String,
                       val key: String,
                       val type: String,
                       val isActive: Boolean,
                       val reducesHolidayEntitlement: Boolean)

data class Absence(val name: String,
...
data class GetPresentPersonsOfOrganizationUnitRq(val organizationUnitNumber: String)
data class GetAllActivePersonsRq(val organizationUnits: List<String>)
data class GetAvailableWorkingHoursRq(val beginDate: Date,
                                      val endDate: Date,
...
data class GetAllAbsenceTypesRq(val onlyActive: Boolean = true)
data class GetLeaveAccountDataRq(val referenceDate: Date = Date(),
                                 @SerializedName("persons") val idOrNumberList: List<String>,
                                 val usePersonnelNumbers: Boolean = false)

data class GetAbsencesRq(val beginDate: Date,
                         val endDate: Date,
                         @SerializedName("persons")
                         val idOrPersonnelNumberList: List<String>,
                         val interval: IntervalType? = null,
...
data class GetAccumulatedAbsencesRq(val beginDate: Date,
                                    val endDate: Date,
                                    @SerializedName("persons")
                                    val idOrPersonnelNumberList: List<String>,
                                    val interval: IntervalType? = null,
